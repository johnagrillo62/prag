...
module FSharp
  ...
  def self.write(class_def)
    ...
    out = [
      ...
      "namespace db.#{class_def.database}",
      "open Db.Utils",
      "open System",
      "open System.Data.Odbc",
      "open System.Collections.Generic",
      "type #{class_name}(" + class_def.members.map do |m|
        to_parameter(writer, m)
      end * ", " + ") = ",
      "",
      class_def.members.map do |m|
        "  member this." + fix_reserved(writer.to_member_name(m.column_name)) + " = " + self.fix_reserved(writer.parameter_str(m.column_name))
      end * "\n",
      "",
      "  static member getValue (index: int) (reader: OdbcDataReader) =",
      "      if reader.IsDBNull(index) then None",
      "      else Some(reader.GetValue(index))",
      "",
      get_func(writer, class_name, class_def.table_name, class_def.members),
      get_by_func(writer, class_name, class_def.table_name, class_def.members, class_def.uniques),
    ]
      [ "fs" , [[src, out.compact * "\n"]]]
  end
end
