...
private:
    ...
    bhw::Type convertTemplateType(const clang::TemplateSpecializationType* tst, clang::ASTContext& ctx) {
        ...
        else if (templateName == "tuple" || templateName == "pair") genericId = bhw::ReifiedTypeId::Tuple;
...
int main(int argc, char** argv) {
    ...
    // Fix 1: DiagnosticOptions is no longer RefCountedBase in LLVM 21
    // Use std::make_unique or stack allocation instead of IntrusiveRefCntPtr
    DiagnosticOptions diagOpts;
    
    // Fix 2: Create DiagnosticIDs with IntrusiveRefCntPtr (this one is still ref-counted)
    llvm::IntrusiveRefCntPtr<DiagnosticIDs> diagIDs(new DiagnosticIDs());
    
    // Fix 3: DiagnosticsEngine constructor now expects DiagnosticOptions&
    // Pass the raw pointer/reference directly
    DiagnosticsEngine diags(diagIDs, diagOpts, &g_diagConsumer, false);
    ...
    // Fix 4: TargetInfo::CreateTargetInfo expects TargetOptions&
    // Create TargetOptions on the stack and pass by reference
    TargetOptions targetOpts;
    targetOpts.Triple = "x86_64-unknown-linux-gnu"; // Default triple
    
    TargetInfo *targetInfo = TargetInfo::CreateTargetInfo(diags, targetOpts);
    if (!targetInfo) {
    ...
    std::cout << "Successfully parsed " << g_structs.size() << " structs\n";
    return 0;
}
